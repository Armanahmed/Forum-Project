            ...
            <div class="panel panel-info">
               <div class="panel-heading">
                  <div class="card-title">
                     <div class="title">Edit Coordinator</div>
                  </div>
               </div>
               <div class="panel-body">
                  <form action="{{ route('coordinators.update', $coordinator->id) }}" method="POST" data-parsley-validate>
                  {{ csrf_field() }}
                  {{ method_field('PUT') }}
                     <div class="form-group">
                        <label for="name">Coordinator Name</label>
                        <input type="text" class="form-control" id="name" name="name" value="{{strip_tags($coordinator->name)}}" minlength="3" required>
                     </div>

                     <div class="form-group">
                        <label for="email">Email</label>
                        <input type="email" class="form-control" id="email" name="email" value="{{strip_tags($coordinator->email)}}" required>
                     </div>
                     ...
                     <button type="submit" class="btn btn-default btn-info admin-form-button">Update</button>
                     <a href="{{ route('coordinators.show', $coordinator->id) }}" class="btn btn-danger admin-form-button"> Cancel <i class="fa fa-angle-double-right"></i></a>
                  </form>
               </div>
            </div>
         </div>
...
$(document).ready(function() {
      $("#department_id").val("{{ $coordinator->coordinator_id }}").change();
    });
